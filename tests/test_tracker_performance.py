from src.detectionObject import DetectionObject
from src.tracker import EuclideanDistTracker
...
def test_tracker_empty_frame(request):
    ...
    detections_per_frame = [
        [DetectionObject(x=10, y=10, width=5, height=5)],
        [],  # No detections
        [DetectionObject(x=12, y=12, width=5, height=5)],
    ]
    tracked_per_frame = []
    for detections in detections_per_frame:
        tracked = tracker.update(detections)
...
def test_tracker_overlapping_objects(request):
    ...
    detections_per_frame = [
        [
            DetectionObject(x=10, y=10, width=10, height=10),
            DetectionObject(x=100, y=100, width=10, height=10),
        ],
        [
            DetectionObject(x=50, y=50, width=20, height=20),
            DetectionObject(x=55, y=55, width=20, height=20),
        ],  # Overlap
        [
            DetectionObject(x=100, y=100, width=10, height=10),
            DetectionObject(x=10, y=10, width=10, height=10),
...
def test_tracker_many_objects(request):
    ...
    tracker = EuclideanDistTracker()
    detections_per_frame = [
        [DetectionObject(x=10 * i, y=10 * i, width=5, height=5) for i in range(10)],
        [DetectionObject(x=10 * i + 2, y=10 * i + 2, width=5, height=5) for i in range(10)],
        [DetectionObject(x=10 * i + 4, y=10 * i + 4, width=5, height=5) for i in range(10)],
    ]
    tracked_per_frame = []
    for detections in detections_per_frame:
        tracked = tracker.update(detections)
...
def test_tracker_bounding_box_change(request):
    ...
    detections_per_frame = [
        [DetectionObject(x=10, y=10, width=5, height=5)],
        [DetectionObject(x=12, y=10, width=10, height=8)],  # Size/shape change
        [DetectionObject(x=15, y=12, width=7, height=6)],
    ]
    ids = []
    tracked_per_frame = []
    for detections in detections_per_frame:
...
def test_tracker_out_of_bounds(request):
    ...
    tracker = EuclideanDistTracker()
    detections_per_frame = [
        [DetectionObject(x=-10, y=-10, width=5, height=5)],  # Negative coords
        [DetectionObject(x=410, y=410, width=5, height=5)],  # Outside window
        [DetectionObject(x=10, y=10, width=5, height=5)],
    ]
    tracked_per_frame = []
    for detections in detections_per_frame:
